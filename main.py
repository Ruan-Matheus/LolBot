...
def get_player_data(summoner_name, api_key):
    try:
        summomer_request = requests.get(f'https://br1.api.riotgames.com/lol/summoner/v4/summoners/by-name/{summoner_name}?api_key={api_key}')
    ...
    try:
        ...
        player_status = {
            ...
            'Win Rate': round(entries_json[n_solo]['wins'] /(entries_json[n_solo]['wins'] + entries_json[n_solo]['losses']) * 100, 2) if entries_json else 0
        }
...
def format_data(player_data):
    if player_data:
        return f"{player_data['Name']} - {player_data['Tier']} {player_data['Rank']} - {player_data['LP']} LP\n - Wins: {player_data['Wins']}\n- Losses: {player_data['Losses']}\n- Win Rate: {player_data['Win Rate']}%"
